...
def clustering_phase_creating_train_dataset_dictionary():
    raw_dataset = english_tokenizer.DataSet(os.path.join('Data', 'phase2_train.csv'), has_tag=True)
...
def clustering_phase_creating_test_dataset_pickle_file():
    ...
    file = open(os.path.join('Data', 'phase2_test') + ".pkl", "wb")
    pickle.dump(output, file)
    file.close()


def clustering_phase_creating_test_dataset_csv_file():
    file = open(os.path.join('Data', 'phase2_test.csv') + ".pkl", "rb")
    ...
    for i in range(len(output)):
        ...
        output_text += "\n"
    file = open(os.path.join('Data', 'test_data.csv'), "w")
...
def clustering_phase_creating_train_dataset_pickle_file():
    raw_dataset = english_tokenizer.DataSet(os.path.join('Data', 'phase2_train.csv'), has_tag=True)
    output = clustering.creating_data_set(raw_dataset.data)
    file = open(os.path.join('Data', 'phase2_train.csv') + ".pkl", "wb")
    pickle.dump(output, file)
    file.close()


def clustering_phase_creating_train_dataset_csv_file():
    file = open(os.path.join('Data', 'phase2_train') + ".pkl", "rb")
    output = pickle.load(file)
    file.close()
    output_arr = []
    for i in range(len(output)):
        ...
        for j in range(len(output[i])):
            this_output += str(output[i][j])
            this_output += ","
        this_output = this_output[0:-1]
        this_output += "\n"
        output_arr.append(this_output)
    output_text = "".join(output_arr)
    file = open(os.path.join('Data', 'train_data.csv'), "w")
    file.write(output_text)
    file.close()


def clustering_phase_test_dataset_with_train_dictionary_pkl_file():
    raw_dataset = english_tokenizer.DataSet(os.path.join('Data', 'phase2_test.csv'), has_tag=True)
    output = clustering.creating_dataset_with_dictionary(raw_dataset.data, "train_dictionary")
    file = open(os.path.join('Data', 'phase2_test_with_train_dictionary') + ".pkl", "wb")
    pickle.dump(output, file)
    file.close()


def clustering_phase_creating_test_dataset_with_train_dictionary_csv_file():
    file = open(os.path.join('Data', 'phase2_test_with_train_dictionary') + ".pkl", "rb")
    output = pickle.load(file)
    file.close()
    output_arr = []
    for i in range(len(output)):
        ...
        output_arr.append(this_output)
    output_text = "".join(output_arr)
    file = open(os.path.join('Data', 'test_data_with_train_dictionary.csv'), "w")
    file.write(output_text)
    file.close()
